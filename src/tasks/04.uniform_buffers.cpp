...
fn vs_main(in: VertexInput) -> VertexOutput {
    ...
    var offset = vec2f(-0.6875, -0.463);
    offset += 0.3 * vec2f(cos(uData.time), sin(uData.time));
...
@fragment
fn fs_main(in: FragmentInput) -> @location(0) vec4f {
    return vec4f(in.color, 1.0) * uData.color;
}
)";
...
constexpr std::initializer_list<runtime::graphics_engine::Layout const> layouts {{
    .bindingPoint = task1::UBO_BINDING_POINT,
...
struct Renderer : public core::Renderer {
    ...
    Renderer(runtime::graphics_engine::IResourceManager& manager)
        : pipeline(manager.createRenderPipeline({}, shaderSource, task1::attributes, layouts))
        , mesh(manager.createMesh(std::span{task1::indexData}, std::span{task1::vertexData}))
        , ubo(manager.createUniformBuffer(UniformData{}))
    {}

    void render(FrameContext const& context) override {
        rewrite(*ubo, UniformData{{1.0f, 0.5f, 0.0f, 1.0f}, context.renderTime});
        auto& encoder = context.renderer.renderPassEncoder();
        encoder.bindRenderPipeline(*pipeline);
        encoder.bind(*ubo, task1::UBO_BINDING_POINT);
        encoder.draw(*mesh);
    }
...
struct : TaskRegistry {
    std::string_view name() const noexcept override { return "008 Uniform buffer v+f"; }
    RendererCreator creator() const noexcept override { return Renderer::create; }
} _;
